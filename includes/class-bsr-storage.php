...
class BSR_Storage {
	 ...
	 * @return string
	 */
	private static function chunk_for( $minute ) {
		return self::CHUNK_PREFIX . gmdate( 'YmdH', $minute * 60 );
	}

	/**
	 * @param array $row Must carry 'm' (minute id).
	 */
	public static function add_minute( array $row ) {
		$minute = (int) $row['m'];
		$name   = self::chunk_for( $minute );
		...
		$index = get_option( self::CHUNK_INDEX, [] );
		...
		if ( ! in_array( $name, $index, true ) ) {
			...
			update_option( self::CHUNK_INDEX, $index, false );
		}
	}
	 ...
	 * @return array
	 */
	public static function minutes_last( $n, $until = null ) {
		$until = null === $until ? BSR_Helpers::minute() - 1 : (int) $until;
		$from  = $until - (int) $n + 1;
		$rows  = [];
		$seen  = [];
		for ( $m = $from; $m <= $until; $m += 60 ) {
			$name = self::chunk_for( $m );
			$seen[ $name ] = true;
		}
		$seen[ self::chunk_for( $until ) ] = true;
	 ...
	 * @return array
	 */
	public static function minutes_for_day( $ymd ) {
		$rows = [];
		for ( $h = 0; $h < 24; $h++ ) {
			$chunk = get_option( self::CHUNK_PREFIX . $ymd . sprintf( '%02d', $h ), [] );
			if ( is_array( $chunk ) ) {
				foreach ( $chunk as $m => $row ) {
					$rows[ (int) $m ] = $row;
				}
	 ...
	 */
	public static function add_transition( array $t ) {
		$log = get_option( self::TRANSITIONS, [] );
		$log = is_array( $log ) ? $log : [];
		array_unshift( $log, $t );
		if ( count( $log ) > self::TRANSITIONS_CAP ) {
			$log = array_slice( $log, 0, self::TRANSITIONS_CAP );
		}
		if ( false === get_option( self::TRANSITIONS, false ) ) {
			add_option( self::TRANSITIONS, $log, '', false );
		} else {
			update_option( self::TRANSITIONS, $log, false );
		}
	}
	 ...
	 */
	public static function transitions( $n = 50 ) {
		$log = get_option( self::TRANSITIONS, [] );
		return is_array( $log ) ? array_slice( $log, 0, $n ) : [];
	}

	/**
	 * Remove every stored row, transition and chunk (tests, reset).
	 */
	public static function clear() {
		...
		delete_option( self::CHUNK_INDEX );
		delete_option( self::TRANSITIONS );
	}
}
